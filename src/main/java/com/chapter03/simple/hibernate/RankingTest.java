...
public class RankingTest {
    ...
    public void changeRanking() {
        ...
        assertEquals(getAverage("J. C. Smell", "Java"), 8.0);
    }

    private double getAverage(String objectName, String skillName) {
        ...
        try (Session session = factory.openSession()) {
            ...
            Query<Ranking> query = session.createQuery("from Ranking r "
                    + "where r.subject.name=:subject and "
                    + "r.skill.name=:skill", Ranking.class);
            query.setParameter("subject", objectName);
            query.setParameter("skill", skillName);
            rankings = query.list();
            assertNotNull(rankings, "Could not find matching ranking");
            tx.commit();
        }
        return rankings.stream().mapToDouble(Ranking::getRanking).average().getAsDouble();
    }
    ...
    public void removeRanking() {
        ...
        assertEquals(getAverage("J. C. Smell", "Java"), 7.5);
    }
